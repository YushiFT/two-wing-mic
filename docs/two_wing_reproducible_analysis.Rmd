...
load('../output/HZ/pca_bay.RData')
pcs_mu  <- data.frame(t(pca_mu$pc))
pcs_k   <- data.frame(t(pca_k$pc))
pcs_all <- data.frame(t(pca_all$pc))
bay_plt <- rbind(bay_plt, data.frame(PC1=pcs_mu$PC1,
                                     PC2=pcs_mu$PC2,
                                     Site=lab_site,
                                     Wing='mu-wing',
                                     Region='Bay'))
bay_plt <- rbind(bay_plt, data.frame(PC1=pcs_k$PC1,
                                     PC2=pcs_k$PC2,
                                     Site=lab_site,
                                     Wing='k-wing',
                                     Region='Bay'))
bay_plt <- rbind(bay_plt, data.frame(PC1=pcs_all$PC1,
                                     PC2=pcs_all$PC2,
                                     Site=lab_site,
                                     Wing='all',
                                     Region='Bay'))
...
g_bay <- ggplot(bay_plt, aes(x=PC1, y=PC2, color=Site)) +
...
load('../output/HZ/pca_era.RData')
pcs_mu  <- data.frame(t(pca_mu$pc))
pcs_k   <- data.frame(t(pca_k$pc))
pcs_all <- data.frame(t(pca_all$pc))
era_plt <- rbind(era_plt, data.frame(PC1=pcs_mu$PC1,
                                     PC2=pcs_mu$PC2,
                                     Site=lab_site,
                                     Wing='mu-wing',
                                     Region='ERA'))
era_plt <- rbind(era_plt, data.frame(PC1=pcs_k$PC1,
                                     PC2=pcs_k$PC2,
                                     Site=lab_site,
                                     Wing='k-wing',
                                     Region='ERA'))
era_plt <- rbind(era_plt, data.frame(PC1=pcs_all$PC1,
                                     PC2=pcs_all$PC2,
                                     Site=lab_site,
                                     Wing='all',
                                     Region='ERA'))
...
g_era <- ggplot(era_plt, aes(x=PC1, y=PC2, color=Site)) +
  geom_point(size=0.8, alpha=0.8) +
  theme_bw() +
  xlab(TeX('PC1')) +
  ylab(TeX('PC2')) +
...
```{r pca_ao, eval=TRUE, echo=TRUE, warning=FALSE, message=FALSE}

dat_plt <- data.frame()
vec_process <- c('influent','anoxic','oxic','effluent')
lab_site <- c(rep(c('LS','SF','CZ','BA','YF','GB','YTSW','XHC','ZC','YT','YN'), each=3))
for(item in vec_process){
  load(paste0('../output/AO_ASV/pca_',item,'.RData'))
  pcs_mu  <- data.frame(t(pca_mu$pc))
  pcs_k   <- data.frame(t(pca_k$pc))
  pcs_all <- data.frame(t(pca_all$pc))
...
Run `../figures/vignettes/pcoa_hz.Rmd` and `../figures/vignettes/pcoa_ao.Rmd`

#### CCA

Run `../figures/vignettes/cca_ao_ori.Rmd`
...
```{r mu_k_category, echo=FALSE, eval=TRUE, warning=FALSE, message=FALSE}
# load parameters mle
vec_process <- c('influent','anoxic','oxic','effluent')
param_plt <- data.frame()
for(item in vec_process){
